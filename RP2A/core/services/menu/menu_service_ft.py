...
from models.base.menu_models import MenuItem, MenuItemSummarized, OrderType, Special
...
class MenuServiceFT(MenuService):
    ...
    async def get_item_details_summarized(self, item: str, category: str, orderType: str) -> MenuItemSummarized:
        try:
            item_ft = await self._api_client.get_item_details(item, category, orderType)
            return MenuAdapterFT.adapt_menu_item_summarized(item_ft)
        except Exception as exc:
            raise RuntimeError(f"Failed to get item details for item={item}, category={category}, orderType={orderType}: {exc}") from exc

...
async def test_save_workflow_captures_checklist():
    ...
    state["task_checklist"] = TaskChecklist(
        ...
        steps=[
            ChecklistStep(
                "s1", "Fetch data", "done", agent_id="did:orcha:agent:data-001"
            ),
            ChecklistStep(
                "s2", "Send email", "done", agent_id="did:orcha:agent:gmail-001"
            ),
        ],
    )
    ...
    assert "did:orcha:agent:data-001" in result["agents_used"]
    assert "did:orcha:agent:gmail-001" in result["agents_used"]

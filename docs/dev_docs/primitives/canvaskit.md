...
> "shadcn/ui for AI-native apps — but with an economic layer."
...
> **Status: planned for DAPN Plane 4.** The billing infrastructure does not exist yet. Components today earn reputation and adoption; the fee layer activates when the paid DAPN runtime ships.

Every component in the CanvasKit ecosystem is designed to earn when it renders in a deployed DAPN app:
...
A `MetricCard` that renders 10,000 times/day in deployed apps would earn its author automatically — no marketplace listing required, no sales, just usage. This is the design target; the mechanism ships with DAPN Plane 2 (Studio).

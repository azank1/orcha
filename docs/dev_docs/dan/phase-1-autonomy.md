...
Slow path (LLM, invoked only on signal):
  ...
  Record: ExecutionObserver.on_step_complete()
```
  ...
  Observe ──► gossip state, own metrics, knowledge store queries
   ...
   Think ──► LLM reasoning (or rule-based — agent's choice)
    ...
    Act ──► invoke peer agents / emit INTENT_BROADCAST / update knowledge
      │
      ▼
  Record ──► ExecutionObserver.on_step_complete() ← the hook that's already there
      │
      ▼
...
In Phase 1 (experimental), [`FulfillmentRecorder`](../../../services/validator/src/validator/recorder.py) implements this hook:
...
Spike transport: Kafka topic `execution.step_complete` ([`step_events.py`](../../../services/superagent/src/superagent/middleware/step_events.py)). Requires `network.experimental: true` in emerge.yaml until the Day-30 gate. Attestations feed Phase 3 reputation scoring.

...
- **`azank1/orcha-internal`** (private, this repo) — the working repo. All
  branches, full history, `docs-local/`, strategy docs.
- **`azank1/orcha`** (public) — the OSS face. Two refs only:
  - `main` — production code, sanitized orphan export of this repo's `main`.
...
- Export flow: internal `main` → sanitize (brand sweep, exclude internal
  ...
  KY-A content must never cross into the public repo — it is hackathon/product
  work, private by design.

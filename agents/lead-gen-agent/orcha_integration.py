"""
orcha_integration.py
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Shows exactly how orcha integrates the lead gen A2A agent
at runtime — with credentials resolved per-request, per-user.

Drop this file into your orcha orchestrator and call
...
logger = logging.getLogger("orcha.lead_gen")
...
class LeadGenCredentialResolver:
    ...
    your orcha environment — users never see them.
    ...
    async def is_ready(self, user_id: str) -> tuple[bool, list[str]]:
        ...
        if not os.getenv(llm_key_name):
            missing.append(f"{llm_key_name} (platform — set in orcha env)")
...
class LeadGenAgentCaller:
    ...
    async def run(self, user_id: str, tool_input: dict) -> dict:
        """
        Main entry point called by orcha tool dispatcher.
...
# ── 4. Registration — call this in your orcha startup ─────────────────────

def register_lead_gen_agent(app, orchestrator_agent, token_store):
    """
    Register the lead gen agent into orcha at startup.

    app:                your FastAPI app
    orchestrator_agent: your main orcha LLM agent object
    ...
    # Optional direct HTTP endpoints on orcha
...
async def dispatch_tool(tool_name: str, tool_input: dict, user_id: str, caller: LeadGenAgentCaller) -> Any:
    """
    Add this case to your existing orcha tool dispatcher.
...
# ── 6. Minimal working example — paste into your orcha main.py ────────────

"""
# In your orcha main.py:

from orcha_integration import register_lead_gen_agent, LeadGenCredentialResolver
...
# 1. orcha env holds: ANTHROPIC_API_KEY, APOLLO_API_KEY, HUNTER_API_KEY
...
# 5. Agent executes → result back to orcha → back to user
"""
